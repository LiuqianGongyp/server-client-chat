...
bool MysqlConnection::update(const std::string &sql) {
    if(mysql_query(conn_, sql.c_str())){
        LOG_INFO << __FILE__ << ":" << __LINE__ << ":" << sql << "failed!";
        return false;
    }
    return true;
}
...
bool MysqlConnection::resultNotEmpty() {
    if(result_ != nullptr){
        return true;
    }
    return false;
...
my_ulonglong MysqlConnection::getLastInsertid() {
    my_ulonglong insertid = mysql_insert_id(conn_);
    if(insertid == 0){
        LOG_INFO << "NO AUTO_INCREMENT ID generated";
    }
    return insertid;
}
//
